...
if __name__ == '__main__':
    ...
    LEARNING_RATE = 0.03  # 学习率 炼丹炉  参数搜索
    ...
    pre0_data[:, 2] = classification  # 第零次对标签的预测情况
    plot_data(pre0_data, '还没训练的预测情况')  # 瞎蒙的

    # 整个网络的反向传播
    backup_network = network.network_backward(outputs, data_label)  # 备用网络
    new1_outputs = backup_network.network_forward(data_feature)  # 备用网络的前馈运算
    new1_classification = classify(new1_outputs[-1])
    print('\n预测的概率转化为标签')
    print(new1_classification)
    pre1_data = np.copy(data)
    pre1_data[:, 2] = new1_classification  # 第一次对标签的预测情况
    plot_data(pre1_data, '训练过一次的预测情况')
