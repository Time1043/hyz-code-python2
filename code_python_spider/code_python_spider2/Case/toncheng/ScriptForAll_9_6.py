...
def getCommentPageNumber(spotCodeList):
    ...
    for spotCode in spotCodeList:
        ...
        #先获取评论的第一页,查看他总共有几页
        commentUrl = "https://www.ly.com/scenery/AjaxHelper/DianPingAjax.aspx?action=GetDianPingList&sid={spotid}&page={page}&pageSize=10&labId=1&sort=0".format(spotid=spotCode, page=1)
        ...
        urlDict[("https://www.ly.com/" + spotCodeList.get(spotCode),spotCode)] = getPageIndexList(pageNumber=int(pageNumber),percent=0.2)
     #将最终结果的字典返回
    return urlDict
...
def getSpotCode(sliceUrl):
    ...
    for urlEnding in sliceUrl:
        ...
        #获取景区id
        spotId.append(re.compile("https://www.ly.com//scenery/BookSceneryTicket_(?P<id>.*?).html", re.S).findall(secondUrl)[0])
    return spotId
...
def transferPage(page,pageNumber):
    ...
    #若不为1,则跳转,并获取页面url
    if pageNumber == 1:
        splitUrl = etree.HTML(str(driver.page_source)).xpath("//div[@class='scenery_content']//a[@class='sce_name goFinal']//@href")
        return splitUrl
    else:
...
def getInformation(zoneIndex,gradeIndex):
    ...
    time.sleep(0.5)
    #依次点击地区
    zone = driver.find_element(By.XPATH,'//*[@id="content"]/div[3]/div[3]/div/div/div[2]/div[1]/a[{zoneIndex}]'.format(zoneIndex = zoneIndex)).text
    driver.find_element(By.XPATH,'//*[@id="content"]/div[3]/div[3]/div/div/div[2]/div[1]/a[{zoneIndex}]'.format(zoneIndex = zoneIndex)).click()
    time.sleep(0.5)
    #获取景区,点等级,建议从1开始
    grade = driver.find_element(By.XPATH,'//*[@id="content"]/div[3]/div[5]/div/span[3]/a[{gradeIndex}]'.format(gradeIndex = gradeIndex)).text
    driver.find_element(By.XPATH,'//*[@id="content"]/div[3]/div[5]/div/span[3]/a[{gradeIndex}]'.format(gradeIndex = gradeIndex)).click()
    time.sleep(0.5)
...
def getSpotInformation(url,headers):
    ...
    try:
        ...
        informationDict.update({
            ...
            "address": etree.HTML(informationPage).xpath('//*[@id="content"]/div[3]/div[2]/p[1]/span/text()')[0].replace('\n', "").replace(" ", "").replace("\'",""),
            "openTime": etree.HTML(informationPage).xpath('//*[@id="content"]/div[3]/div[2]/p[2]/span/text()')[0].replace('\n', "").replace(" ", "").replace("\'",""),
            "ticket":etree.HTML(informationPage).xpath('//*[@id="content"]/div[3]/div[2]/div[6]/div/span/b/text()')[0].replace('\n', "").replace(" ", "").replace("\'",""),
            "introduce":etree.HTML(informationPage).xpath('//*[@id="scenery_intro_con"]/div[2]/div/div[1]/div/p/text()')[0].replace('\n', "").replace(" ", "").replace("\'","")
        })
        try:
            imgUrl = "https:"+etree.HTML(informationPage).xpath('//*[@id="scenery_intro_con"]/div[2]/div/div[1]/div/div/img/@orisrc')[0]
            #将文件名和图片url传过去
    ...
    for url,code in urlCodeAndPage:
        ...
        if spotInformation != 1:
            ...
            for commentPage in urlCodeAndPage[(url,code)]:
                ...
                commentUrl = "https://www.ly.com/scenery/AjaxHelper/DianPingAjax.aspx?action=GetDianPingList&sid={spotid}&page={page}&pageSize=10&labId=1&sort=0".format(spotid=code, page=commentPage)
                #获取到一个有关评论的字典
                ...
                try:
                    ...
                    for dic in commentPage:
                        ...
                        input_key = ("commentType","dpGuid","dpId","DPItemId",'DPItemName','memberHeaderImgUrl','dpImgUrl')
                        #将景点和评论字段合二为一
...
def checkCondition(zone,grade):
    ...
    if list(information).__contains__(zone + "|" + grade) : return False
    else: return True
            ...
            for grades in gradeList:
                ...
                if checkCondition(zone,grade):
                    ...
                    for page in getPageIndexList(int(pageNumber),0.1):
                        ...
                        getCommentInformation(getCommentPageNumber(zipToDict(cityCodeList,sliceUrl)),zoneAndGrade={"zone":zone,"grade":grade})
                    # 一轮结束,将结束结果存储入日志当中
