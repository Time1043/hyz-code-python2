...
def reqresp(request):  # request对象 封装了用户通过浏览器发送过来的所有请求数据
    ...
    # 接受用户在url上传递一些值
    print(request.GET)  # <QueryDict: {'n': ['123'], 'p': ['asd']}>
    # 【响应】将指定内容响应
    # return HttpResponse('返回内容')
    # 【响应】将指定内容响应  读取html + 渲染替换 -> 字符串
    # return render(request, 'reqresp.html', {'np': request.GET})
    # 【响应】重定向到其他网页
...
def orm(request):
    ...
    UserInfo.objects.create(name='林云', password='123')
...
def info_list(request):
    data_list = UserInfo.objects.all()  # 数据库取数据
    return render(request, 'info_list.html', {'data_list': data_list})  # 渲染
...
def info_add(request):
    ...
    return redirect('/info/list/')  # 自己家的域名不用写


def info_dlt(request):
